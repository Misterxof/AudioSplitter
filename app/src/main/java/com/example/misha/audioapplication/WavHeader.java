...
/**
 * Class WavHeader for right recoding audio in .wav format
 */
final class WavHeader {
  ...
  /**
   * The constructor of class WavHeader
   * @param pullableSource - source that will be pulled
   * @param totalAudioLength - audio length
   */
  ...
  /**
   * The method returns the {@code WavHeader} in bytes
   * @return wavFileHeader - header
   */
  public byte[] toBytes() {
    long sampleRateInHz = pullableSource.frequency();
    int channels =
        (pullableSource.channelPositionMask() == AudioFormat.CHANNEL_IN_MONO ? 1 : 2);
    byte bitsPerSample = pullableSource.bitsPerSample();
  ...
  /**
   * The method returns the header
   * @param totalAudioLen - total audio length
   * @param totalDataLen - total data length
